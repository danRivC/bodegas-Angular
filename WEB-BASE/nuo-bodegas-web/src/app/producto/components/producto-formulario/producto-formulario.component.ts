...
export class ProductoFormularioComponent implements OnInit, OnChanges {
  ...
  constructor(private router:Router, private _formBuilder:FormBuilder, private _location: Location,
    private productoService: ProductoService, private snackBar: MatSnackBar, private bodegaService:BodegasService,
  ...
  ngOnInit() {
    if(sessionStorage.getItem('codigoProducto')){
      this.productoService.traerProducto(parseInt(sessionStorage.getItem('codigoProducto'))).subscribe(datos=>this.rellenarProducto(datos))
    }
  ...
  guardarProducto(){
    if(this.productoForm.valid){
      ...
      this.productoService.insertarActualizarProducto(producto).subscribe(datos=>this.confirmacion(datos), error=>console.log(error));
    }else{
