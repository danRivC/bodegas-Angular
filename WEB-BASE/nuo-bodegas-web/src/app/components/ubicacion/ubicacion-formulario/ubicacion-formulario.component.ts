...
export class UbicacionFormularioComponent implements OnInit {
  ...
  constructor(private router:Router, private _formBuilder:FormBuilder, private _location: Location, private _ubicacion: UbicacionService, private _snackBar: MatSnackBar)
  {
    this.buildForm();
  }
  private buildForm(){
    this.ubicacionForm= this._formBuilder.group({
  ...
  ngOnInit() {
    ...
    if(sessionStorage.getItem('codigoUbicacion')){
      this._ubicacion.traerUbicacion(parseInt(sessionStorage.getItem('codigoUbicacion'))).subscribe(datos=>this.rellenarCiudad(datos))
    }
  }
  ngOnDestroy(){
  ...
  guardarUbicacion(){
    if(this.ubicacionForm.valid){
      ...
      this._ubicacion.insertarActualizarUbicaciones(ciudad).subscribe(datos=>this.confirmacion(datos), error=>console.log(error))
    }else{
