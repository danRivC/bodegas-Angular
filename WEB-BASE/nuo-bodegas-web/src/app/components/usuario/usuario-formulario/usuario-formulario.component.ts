...
import { PerfilResponse } from 'src/app/components/perfil/models/perfil-response.model';
import { StepperSelectionEvent } from '@angular/cdk/stepper';
import { PerfilUsuario } from 'src/app/components/perfil/models/perfil-usuario.model';
...
export class UsuarioFormularioComponent implements OnInit, OnDestroy {
  ...
  perfilNoDisponible: Array<any>;
    constructor(private _location: Location, private _ciudadService: CiudadService, private _snackBar: MatSnackBar, private _usuario: UsuarioService, private formBuilder: FormBuilder, private route:Router) {
      this.buildForm();
      this.buildFormPerfiles();
     }
     ngOnInit() {
       if(localStorage.getItem('codigoUser')){
        this._usuario.obtenerUnUsuario(parseInt(localStorage.getItem('codigoUser'))).subscribe(datos=>this.rellenarUsuario(datos), error=>this.openSnackBar(error, "cerrar"));
       }else{
         this.limpiar()
       }
  ...
  cargarPerfiles(codigoUser:number){
    this._usuario.obtenerPerfilesUsuario(Number.parseInt(localStorage.getItem('codigoUser'))).subscribe(perfiles=>this.rellenarPerfiles(perfiles), error=>console.log(error))
  }
  ...
  guardar(){
    if(this.userForm.valid){
      ...
      this._usuario.guardarUsuario(this.usuario).subscribe(confirmacion=>this.confirmacion(confirmacion),
  ...
  pasarUno(){
    ...
    if(this.usuarioPerfilForm.value.perfilesDisponibles>0){
      ...
      this._usuario.insertarActualizarPerfilUsuario(usuarioModel).subscribe(datos=>console.log(datos), error=>console.log(error));
  ...
  regresarUno(){
    ...
    if(this.usuarioPerfilForm.value.perfilesNoDisponibles>0){
      ...
      this._usuario.insertarActualizarPerfilUsuario(usuarioModel).subscribe(respuesta=>console.log(respuesta), error=>console.log(error));
      this.cargarPerfiles(Number.parseInt(localStorage.getItem('codigoUser')));
